## 摘要
Pico 的上下文瘦身不是简单地把 prompt 从尾部截断，而是做了一套“分层瘦身”机制。它从内容进入系统开始，到最终组装 prompt，一共做四层控制：

1. **入口层先裁长输出**
    所有工具返回值先通过 clip() 控制长度，默认最多 4000 字符。read_file 结果还会进一步压成前三行摘要，避免完整文件内容在系统内部反复流动。
    
2. **memory 层自己限额**
    memory 不当成第二份历史，而是保持轻量。最近文件最多 8 个，episodic notes 最多 12 条，文件摘要最多 6 条。每条内容也有长度限制，比如 task_summary 300 字符，note 500 字符。
    
3. **history 按新旧分层压缩**
    最近 6 条历史保留较多细节，单条最多 900 字符；更旧的历史压到很短，单条最多 60 字符。旧历史里的重复 read_file 会按文件路径去重，避免同一个文件内容反复塞进 prompt。
    
4. **最终按 section 预算收缩**  
    最终 prompt 被拆成 prefix、memory、relevant_memory、history、current_request 五段，总预算默认 12000 字符。超预算时，按固定顺序裁剪：先裁 relevant_memory，再裁 history，再裁 memory，最后才裁 prefix。current_request 永远不裁。
    

这套机制的核心原则是：当前用户请求、系统规则这些高优先级信息不能丢；旧日志、重复读取、相关记忆这些可以压缩的信息先让位。
